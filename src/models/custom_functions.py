...
class SafePower(BaseSafeFunction):
    ...
    It will only work for 1D input
    """
    def __init__(self):
    ...
    def forward(self, x, weight, sign_param):
        ...
        input_signs = torch.sign(x)  # shape: (N, D)

        # Compute x^w using log-exp for stability
        log_x = torch.log(x_abs)  # shape: (N, D)
        
        # Compute power_result without unnecessary dimensions
        power_result = torch.exp(log_x @ weight.t())  # shape: (N, 1)

        # If any weight is 0, subtract 1 from power_result to handle x^0 = 1 case
        if torch.any(weight == 0):
            power_result = power_result - 1

        # Obtain continuous factor from sign_param
        sign_factor = self.hardsigmoid(sign_param)  # shape: (1,)
        
        if not self.training:
            sign_factor = (sign_factor > 0.5).float()

        # Calculate final sign without extra dimensions
        final_sign = sign_factor * torch.ones_like(input_signs[:, 0]) + (1 - sign_factor) * input_signs[:, 0]
        
        # Compute final output maintaining correct shape
        power_out = (final_sign * power_result).unsqueeze(1) + self.bias  # shape: (N, 1)
...
class SafePower2(BaseSafeFunction):
    """
    Safe power function with trainable exponents and sign handling.
    Implements (x+y)^p where p is trainable and handles negative inputs safely.
    It will work for any input size
    """
    def __init__(self):
        super().__init__("power", "pow")
        self.hardsigmoid = nn.Hardsigmoid()
