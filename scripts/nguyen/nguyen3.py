...
def main():
    ...
    output_size = 1
    num_layers = 2
    nonlinear_info = [(5, 0), (0, 0), (0, 0)]

    # Get data loaders using the new utility function
    train_loader, val_loader = get_nguyen_data_loaders('Nguyen-3', batch_size=64)
    
    # Get the full dataset for plotting
    X, y = generate_nguyen_data('Nguyen-3')

    # Initialize model
    model = initialize_model(input_size, output_size, num_layers, hyp_set, nonlinear_info, min_connections_per_neuron=1, exp_n=3)
